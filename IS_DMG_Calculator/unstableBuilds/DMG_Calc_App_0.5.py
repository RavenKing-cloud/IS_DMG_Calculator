...
class Application(tk.Tk):
    ...
    def run_simulation(self):
        attacker_level = int(self.left_frame.winfo_children()[-1].get())
        defender_level = int(self.right_frame.winfo_children()[-1].get())

        attacker_stats = self.get_entity_stats(self.left_frame)
        defender_stats = self.get_entity_stats(self.right_frame)

        attacker_dmg = self.calculate_damage(attacker_stats, defender_stats, attacker_level, defender_level)
        defender_dmg = self.calculate_damage(defender_stats, attacker_stats, defender_level, attacker_level)

        messagebox.showinfo("Simulation", f"Attacker damage: {attacker_dmg}\nDefender damage: {defender_dmg}")

    def get_entity_stats(self, frame):
        stats_labels = ["Tier", "Rank", "Type", "HP", "MP", "RoA", "BAP", "PA", "SA", "PD", "SD", "DEX"]
        stats = {}

        for label in stats_labels:
            entry = frame.nametowidget(label)
            stats[label] = float(entry.get())

        return stats

    def calculate_damage(self, attacker_stats, defender_stats, attacker_level, defender_level):
        tier = attacker_stats["Tier"]
        rank = attacker_stats["Rank"]
        bap = attacker_stats["BAP"]
        sa = attacker_stats["SA"]
        sd = defender_stats["SD"]
        dex = attacker_stats["DEX"]

        level_diff = attacker_level - defender_level
        if level_diff < 0:
            level_diff = 0

        dmg = (bap * math.pow((2 * tier + rank), 2) * (sa / sd) * (dex / 2) / 100) + level_diff
